...
struct KeybindingsConfig {
    ...
    #[serde(default = "default_layout_panel")]
    layout_panel: String,
    #[serde(default = "default_hide_panel")]
    hide_panel: String,
    ...
    reset: String,
    #[serde(default = "default_item_up")]
    item_up: String,
    #[serde(default = "default_item_down")]
    item_down: String,
    #[serde(default = "default_item_enter")]
    item_enter: String,
    #[serde(default = "default_item_back")]
    item_back: String,
}
    ...
    fn default() -> Self {
        Self {
            ...
            cycle_log_view: default_cycle_log_view(),
                            ...
                            layout_panel: default_layout_panel(),
                            hide_panel: default_hide_panel(),
                            filter: default_filter(),
                            master: default_master(),
                            reset: default_reset(),
                            item_up: default_item_up(),
                            item_down: default_item_down(),
                            item_enter: default_item_enter(),
                            item_back: default_item_back(),
                        }
                    }
                }
                ...
                fn default_back()           -> String { "left".to_string() }
                fn default_run()            -> String { "r".to_string() }
                fn default_stop()           -> String { "x".to_string() }
                fn default_quit()           -> String { "q".to_string() }
                fn default_settings()       -> String { "k".to_string() }
                fn default_favorite()       -> String { "f".to_string() }
                ...
                fn default_next_pane()      -> String { "shift-right".to_string() }
                fn default_prev_pane()      -> String { "shift-left".to_string() }
                ...
                fn default_toggle_selection() -> String { " ".to_string() }
                fn default_layout_panel()   -> String { "l".to_string() }
                fn default_hide_panel()     -> String { "h".to_string() }
                fn default_filter()         -> String { "/".to_string() }
                fn default_master()         -> String { "m".to_string() }
                fn default_reset()          -> String { "l".to_string() }
                fn default_item_up()        -> String { "up".to_string() }
                fn default_item_down()      -> String { "down".to_string() }
                fn default_item_enter()     -> String { "enter".to_string() }
                fn default_item_back()      -> String { "left".to_string() }
                // ── Layout AST ───────────────────────────────────────────────────────────────
...
impl LayoutNode {
    ...
    pub(crate) fn render(&self, area: Rect, app: &App, out: &mut Vec<(Rect, Pane)>) {
        match self {
            LayoutNode::Pane(p) => {
    ...
    pub(crate) fn has_visible(&self, app: &App) -> bool {
        match self {
...
struct LayoutConfig {
    ...
    #[serde(default = "bool_true")]  show_disks: bool,
    #[serde(default = "bool_true")]  show_explorer: bool,
}
...
fn lc_default_tree() -> LayoutNode {
    LayoutNode::Split {
        ...
        a: Box::new(LayoutNode::Split {
            ...
            ratio: 60,
            a: Box::new(LayoutNode::Pane(Pane::Explorer)),
            b: Box::new(LayoutNode::Split {
                dir: SplitDir::Vertical,
                ratio: 50,
                a: Box::new(LayoutNode::Pane(Pane::Favorites)),
...
impl Default for LayoutConfig {
    fn default() -> Self {
        Self {
            ...
            show_logs: true, show_running: true, show_disks: true, show_explorer: true,
        }
    }
}
...
impl Config {
    fn load() -> (Self, PathBuf) {
        ...
        if let Some(path) = config_path {
            ...
            let _ = fs::write(&target_path, DEFAULT_CONFIG);
            
            let cfg = toml::from_str::<Config>(DEFAULT_CONFIG).unwrap_or_else(|_| Self::default());
            (cfg, target_path)
        }
    }
}
...
struct App {
    ...
    show_master_modal: bool,
    master_modal_input: String,
    master_modal_cursor: usize,
    master_modal_state: ListState,
}
...
impl App {
    fn new(scan_dir: PathBuf, config: Config, config_path: PathBuf, bg_tx: mpsc::UnboundedSender<BgMsg>) -> Self {
        ...
        let mut app = Self {
            ...
            drives_cursor: 0,
            drives_state,
                show_disks: config.layout.show_disks,
                show_explorer: config.layout.show_explorer,
            ...
            restart_cooldowns: HashMap::new(),
                ...
                master_modal_input: String::new(),
                master_modal_cursor: 0,
                master_modal_state: ListState::default(),
            };
    ...
    fn save_layout(&mut self) {
        ...
        self.config.layout.show_disks = self.show_disks;
        self.config.layout.show_explorer = self.show_explorer;
    ...
    fn refresh_favorites(&mut self) {
        ...
        for path_str in &self.config.general.favorites {
            let path = PathBuf::from(path_str);
            if path.exists() {
                let name = path.file_name().and_then(|n| n.to_str()).unwrap_or("Unknown").to_string();
                
                // Detect type for favorites too
                let project_type = if path.join("package.json").exists() {
                    ProjectType::Node(PackageManager::detect(&path))
                } else if path.join("Cargo.toml").exists() {
                    ProjectType::Rust
                } else if path.join("requirements.txt").exists() || path.join("main.py").exists() {
                    ProjectType::Python
                } else if path.join("go.mod").exists() {
                    ProjectType::Go
                } else if path.join("artisan").exists() {
                    ProjectType::PHP
                } else if path.join("compose.yaml").exists() || path.join("docker-compose.yml").exists() {
                    ProjectType::Docker
                } else {
                    ProjectType::Unknown
                };
        ...
        } else if self.favorites_cursor >= self.favorites.len() {
            self.favorites_cursor = self.favorites.len() - 1;
            self.favorites_state.select(Some(self.favorites_cursor));
        } else {
            self.favorites_state.select(Some(self.favorites_cursor));
        }
    }

    fn add_remove_favorite(&mut self) {
        let target_path = match self.active_pane {
            Pane::Explorer => self.projects.get(self.cursor).map(|p| p.path.clone()),
            Pane::Favorites => self.favorites.get(self.favorites_cursor).map(|p| p.path.clone()),
            _ => None,
        };

        if let Some(path) = target_path {
            let path_str = path.to_string_lossy().to_string();
            if let Some(pos) = self.config.general.favorites.iter().position(|r| r == &path_str) {
                self.config.general.favorites.remove(pos);
                self.add_system_log(format!("⭐ Removed from favorites: {}", path_str));
            } else {
                self.config.general.favorites.push(path_str.clone());
                self.add_system_log(format!("⭐ Added to favorites: {}", path_str));
            }
    ...
    fn move_down(&mut self) {
        match self.active_pane {
            ...
            Pane::Favorites => {
                if !self.favorites.is_empty() && self.favorites_cursor < self.favorites.len() - 1 {
                    self.favorites_cursor += 1;
                    self.favorites_state.select(Some(self.favorites_cursor));
                }
            }
    ...
    fn toggle_running_panel(&mut self) {
        ...
        if !self.show_running && self.active_pane == Pane::Running {
            self.active_pane = Pane::Explorer;
        }
    }
    ...
    fn is_visible(&self, pane: Pane) -> bool {
        match pane {
            Pane::Explorer => self.show_explorer,
            Pane::Logs => self.show_logs,
            Pane::Running => self.show_running,
            Pane::Sidebar => self.show_disks,
            Pane::Favorites => self.show_favorites,
        }
    }
    ...
    fn toggle_master_modal(&mut self) {
        self.show_master_modal = !self.show_master_modal;
        self.master_modal_input.clear();
        self.master_modal_cursor = 0;
        self.master_modal_state.select(None);
    }

    fn get_master_commands(&self) -> Vec<(&'static str, &'static str)> {
        vec![
            ("hide", "Hide active panel"),
            ("disks", "Toggle disks"),
            ("running", "Toggle running panel"),
            ("logs", "Toggle logs panel"),
            ("favorites", "Toggle favorites panel"),
    ...
    fn execute_master_command(&mut self, cmd: &str) -> bool {
        match cmd {
            "hide" => { self.hide_active_panel(); true }
            "disks" => { self.toggle_disks(); true }
            "running" => { self.toggle_running_panel(); true }
            "logs" => { self.show_logs = !self.show_logs; true }
            "favorites" => { self.show_favorites = !self.show_favorites; true }
            "explorer" => { self.toggle_explorer(); true }
            "reset" => { self.layout_active_panel(); true }
            "settings" => { self.open_settings(); true }
            "quit" => { false } // We'll handle quit in the event loop
            _ => false,
        }
    }
    ...
    fn hide_active_panel(&mut self) {
        match self.active_pane {
            ...
            Pane::Sidebar => { self.show_disks = false; self.active_pane = Pane::Explorer; }
            Pane::Logs    => { self.show_logs    = false; self.active_pane = Pane::Explorer; }
            Pane::Running => {
                self.show_running = false;
                self.active_pane = if self.show_logs { Pane::Logs } else { Pane::Explorer };
            }
            Pane::Favorites => { self.active_pane = Pane::Explorer; }
        }
    }
    ...
    fn toggle_explorer(&mut self) {
        ...
        if !self.show_explorer && self.active_pane == Pane::Explorer {
            self.active_pane = if self.show_logs { Pane::Logs } else if self.show_running { Pane::Running } else if self.show_disks { Pane::Sidebar } else { Pane::Favorites };
        }
    }

    fn toggle_disks(&mut self) {
        self.show_disks = !self.show_disks;
        if !self.show_disks && self.active_pane == Pane::Sidebar {
            self.active_pane = if self.show_explorer { Pane::Explorer } else if self.show_logs { Pane::Logs } else if self.show_running { Pane::Running } else { Pane::Favorites };
        }
    }

    fn move_pane_up(&mut self) {
        self.active_pane = match self.active_pane {
            Pane::Explorer | Pane::Favorites => Pane::Explorer,
            Pane::Logs | Pane::Running => {
                if self.show_explorer { Pane::Explorer } else { self.active_pane }
            }
            Pane::Sidebar => Pane::Explorer,
        };
    }
    ...
    fn move_pane_down(&mut self) {
        self.active_pane = match self.active_pane {
            Pane::Explorer => {
                if self.show_running { Pane::Running } else if self.show_logs { Pane::Logs } else { Pane::Explorer }
            }
            Pane::Logs | Pane::Running => {
                if self.show_running { Pane::Running } else { self.active_pane }
            }
            Pane::Sidebar => Pane::Explorer,
            Pane::Favorites => {
                if self.show_running { Pane::Running } else if self.show_logs { Pane::Logs } else { Pane::Explorer }
    ...
    fn move_pane_left(&mut self) {
        self.active_pane = match self.active_pane {
            Pane::Explorer | Pane::Logs | Pane::Running | Pane::Favorites => {
                if self.show_disks { Pane::Sidebar } else { self.active_pane }
            }
            Pane::Sidebar => self.active_pane,
        };
    }
    ...
    fn move_pane_right(&mut self) {
        self.active_pane = match self.active_pane {
            Pane::Sidebar => {
                if self.show_explorer { Pane::Explorer } else { Pane::Sidebar }
            }
            Pane::Explorer | Pane::Logs | Pane::Running | Pane::Favorites => {
                if self.show_logs { Pane::Logs } else if self.show_running { Pane::Running } else { self.active_pane }
            }
        };
    }

    fn prev_pane(&mut self) {
        // Removed as it's no longer used in favor of move_pane_left
    }
    ...
    fn enter_item(&mut self) {
        match self.active_pane {
            ...
            Pane::Favorites => {
                if let Some(p) = self.favorites.get(self.favorites_cursor) {
                    let new_dir = p.path.clone();
                    self.scan_dir = new_dir;
                    self.refresh_list();
                    self.active_pane = Pane::Explorer;
    ...
    fn kill_selected(&mut self) {
        let mut projects_to_kill: Vec<Project> = self.projects.iter().filter(|p| p.selected).cloned().collect();
        
        // If nothing selected but we are in Explorer or Favorites, kill the highlighted one
        if projects_to_kill.is_empty() {
            match self.active_pane {
                Pane::Explorer => {
                    if let Some(p) = self.projects.get(self.cursor).cloned() {
                        if p.name != ".." { projects_to_kill.push(p); }
                    }
                }
                Pane::Favorites => {
                    if let Some(p) = self.favorites.get(self.favorites_cursor).cloned() {
                        projects_to_kill.push(p);
                    }
                }
                _ => {}
            }
    ...
    fn launch_selected(&mut self) {
        let mut projects_to_launch: Vec<Project> = self.projects.iter().filter(|p| p.selected).cloned().collect();
        
        // If nothing selected but we are in Explorer or Favorites, launch the highlighted one
        if projects_to_launch.is_empty() {
            match self.active_pane {
                Pane::Explorer => {
                    if let Some(p) = self.projects.get(self.cursor).cloned() {
                        if p.name != ".." { projects_to_launch.push(p); }
                    }
                }
                Pane::Favorites => {
                    if let Some(p) = self.favorites.get(self.favorites_cursor).cloned() {
                        projects_to_launch.push(p);
                    }
                }
                _ => {}
            }
...
fn render(frame: &mut ratatui::Frame, app: &mut App) {
    ...
    let items = [
        ("Up", kb.item_up.to_uppercase()), ("Down", kb.item_down.to_uppercase()),
        ("Enter", kb.item_enter.to_uppercase()), ("Back", kb.item_back.to_uppercase()),
        ("Sel", kb.toggle_selection.to_uppercase()), ("Run", kb.run.to_uppercase()),
        ("Stop", kb.stop.to_uppercase()), ("Reset", kb.reset.to_uppercase()),
        ("Hide", kb.hide_panel.to_uppercase()), ("Fav", kb.favorite.to_uppercase()),
        ("Orient", kb.toggle_split.to_uppercase()),
        ("Pane", kb.next_pane.to_uppercase()), ("Move", "Ctrl+Arrows".to_string()),
        ("Resize", "Alt+Arrows".to_string()),
    ...
    if app.show_favorites {
        if let Some(f_area) = favorites_area_opt {
            let fav_items: Vec<ListItem> = if app.favorites.is_empty() {
                vec![ListItem::new(Line::from(Span::styled("  No favorites yet", Style::default().fg(Color::DarkGray))))]
            } else {
                app.favorites
                    .iter()
                    .enumerate()
                    .map(|(i, p)| {
                        let row_style = if i == app.favorites_cursor && app.active_pane == Pane::Favorites {
                            Style::default().bg(theme.selection_bg).add_modifier(Modifier::BOLD)
                        } else if i == app.favorites_cursor {
                            Style::default().bg(theme.selection_bg)
                        } else {
                            Style::default()
                        };
                        ...
                        Line::from(vec![
                            ...
                            Span::styled(format!("[{}] ", p.project_type.tag()), Style::default().fg(p.project_type.color())),
                            Span::styled(format!("{:<15}", p.name), row_style),
                            Span::styled(format!(" {}", status.label()), Style::default().fg(status_color)),
                        ])
                    })
                    .map(ListItem::new)
                    .collect()
            ...
            let list = List::new(fav_items)
                .block(pane_border(" Favorites ", app.active_pane == Pane::Favorites, &theme))
                .highlight_style(Style::default().bg(theme.selection_bg).add_modifier(Modifier::BOLD));
            frame.render_stateful_widget(list, f_area, &mut app.favorites_state);
        }
    }
    ...
    if app.show_explorer {
        let list_items: Vec<ListItem> = if app.projects.is_empty() {
            ...
            app.projects
                ...
                .map(|(i, p)| {
                    ...
                    let row_style = if i == app.cursor && app.active_pane == Pane::Explorer {
                        Style::default().bg(theme.selection_bg).add_modifier(Modifier::BOLD)
        ...
        let list = List::new(list_items)
            .block(pane_border(" Explorer ", app.active_pane == Pane::Explorer, &theme))
            .highlight_style(Style::default().bg(theme.selection_bg).add_modifier(Modifier::BOLD));
    ...
    if let Some(req) = &app.pending_install {
        ...
        // ── Master Command Modal ───────────────────────────────────────────────────
        if app.show_master_modal {
            use ratatui::widgets::Clear;
            
            let modal_w = (area.width / 3).max(40).min(area.width.saturating_sub(4));
            let modal_h = 15u16;
            ...
            let filtered_cmds: Vec<(&str, &str)> = app.get_master_commands()
                .into_iter()
                .filter(|(cmd, _)| cmd.to_lowercase().contains(&app.master_modal_input.to_lowercase()))
                .collect();

            let cmd_list: Vec<ListItem> = filtered_cmds
                .iter()
                .enumerate()
                .map(|(i, (cmd, desc))| {
                    let style = if i == app.master_modal_cursor {
                        Style::default().bg(theme.selection_bg).add_modifier(Modifier::BOLD)
                    } else {
                        Style::default()
                    };
                    ListItem::new(Line::from(vec![
                        Span::styled(format!("{:<10}", cmd), Style::default().fg(theme.active_border).add_modifier(Modifier::BOLD)),
                        Span::styled(format!(" - {}", desc), style),
                    ]))
                })
                .collect();

            let list = List::new(cmd_list)
                .block(Block::default().borders(Borders::ALL).title(" Master Commands ").border_style(Style::default().fg(theme.active_border)))
                .highlight_style(Style::default().bg(theme.selection_bg));

            // Input area
            let input_text = format!(" 🔍 {} ", app.master_modal_input);
            let input_bar = Paragraph::new(input_text)
                .block(Block::default().borders(Borders::ALL).title(" Search command "))
                .style(Style::default().fg(theme.path_fg));
            ...
            frame.render_stateful_widget(list, layout[1], &mut app.master_modal_state);
        }
    }
        ...
        tokio::select! {
            ...
            Some(key) = key_rx.recv() => {
                ...
                if app.show_master_modal {
                    match code {
                        KeyCode::Char(c) => {
                            app.master_modal_input.push(c);
                        }
                        KeyCode::Backspace => {
                            app.master_modal_input.pop();
                        }
                        KeyCode::Up => {
                            if app.master_modal_cursor > 0 {
                                app.master_modal_cursor -= 1;
                                app.master_modal_state.select(Some(app.master_modal_cursor));
                            }
                        }
                        KeyCode::Down => {
                            let filtered_cmds = app.get_master_commands().into_iter()
                                .filter(|(cmd, _)| cmd.contains(&app.master_modal_input))
                                .collect::<Vec<_>>();
                            if !filtered_cmds.is_empty() && app.master_modal_cursor < filtered_cmds.len() - 1 {
                                app.master_modal_cursor += 1;
                                app.master_modal_state.select(Some(app.master_modal_cursor));
                            }
                        }
                        KeyCode::Enter => {
                            let filtered_cmds = app.get_master_commands().into_iter()
                                .filter(|(cmd, _)| cmd.contains(&app.master_modal_input))
                                .collect::<Vec<_>>();
                            if let Some((cmd, _)) = filtered_cmds.get(app.master_modal_cursor) {
                                if *cmd == "quit" {
                                    app.save_layout();
                                    break;
                                }
                                app.execute_master_command(cmd);
                                app.show_master_modal = false;
                            }
                        }
                        KeyCode::Esc => {
                            app.show_master_modal = false;
                ...
                if key.modifiers.contains(KeyModifiers::CONTROL) && matches!(code, KeyCode::Left | KeyCode::Right | KeyCode::Up | KeyCode::Down) {
                ...
                if key.modifiers.contains(KeyModifiers::SHIFT) && matches!(code, KeyCode::Left | KeyCode::Right | KeyCode::Up | KeyCode::Down) {
                    match code {
                        KeyCode::Up    => app.move_pane_up(),
                        KeyCode::Down  => app.move_pane_down(),
                        KeyCode::Left  => app.move_pane_left(),
                        KeyCode::Right => app.move_pane_right(),
                        _ => {}
                    }
                    app.apply_bg_msgs(bg_rx);
                    terminal.draw(|f| render(f, app))?;
                ...
                if matches_key(&key, &kb.item_up) { app.move_up(); }
                else if matches_key(&key, &kb.item_down) { app.move_down(); }
                else if matches_key(&key, &kb.item_enter) { app.enter_item(); }
                else if matches_key(&key, &kb.item_back) { app.go_back(); }
                else if matches_key(&key, &kb.toggle_selection) { app.toggle_selection(); }
                else if matches_key(&key, &kb.run) { app.launch_selected(); }
                else if matches_key(&key, &kb.stop) { app.kill_selected(); }
                else if matches_key(&key, &kb.layout_panel) { app.layout_active_panel(); }
                else if matches_key(&key, &kb.hide_panel) { app.hide_active_panel(); }
                else if matches_key(&key, &kb.toggle_split) { app.toggle_split_direction(); }
                else if matches_key(&key, &kb.next_pane) { app.move_pane_right(); }
                else if matches_key(&key, &kb.prev_pane) { app.move_pane_left(); }
                ...
                else if matches_key(&key, &kb.filter) {
                    app.is_filtering = true;
                    app.active_pane = Pane::Explorer;
                }
                else if matches_key(&key, &kb.settings) { app.open_settings(); }
                else if matches_key(&key, &kb.favorite) { app.add_remove_favorite(); }
                ...
                else if matches_key(&key, &kb.master) { app.toggle_master_modal(); }
